...
def preprocess_ctpa(img, attr):
    ...
    # Rescale
    img_resize = resize_volume(img_resampled, [128, 128, 128])
...
def preprocess_rspect_directory(src_path, dst_path):
            ...
            for sub, dir, _ in os.walk(sub_path):
                for di in dir: 
                    ...
                    if os.path.isfile(folder_path + '_' + folder_name + '.npy'):
                        ...
                        #sitk.WriteImage(sitk.GetImageFromArray(ct), folder_path + '/' + folder_name + '.nii')
                        np.save(folder_path + '_' + folder_name, latents)
...
if __name__ == "__main__":
    ...
    dst_path = "../datasets/RSPECT/RSPECT_vae_256/"

    preprocess_rspect_directory(src_path, dst_path)
